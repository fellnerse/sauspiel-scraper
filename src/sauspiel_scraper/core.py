...
class SauspielScraper:
    ...
    def get_game_list(
        self, limit: Optional[int] = 10, since: Optional[datetime] = None
    ) -> list[dict[str, Any]]:
        if not self.user_id:
            return []
...
def scrape_games_stream(
    ...
    """
    Scrapes games and yields each game data as a dictionary.
    This allows updating UI components incrementally.
    """
